...
class WaypointUpdater(object):
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            p = Waypoint()
            p.pose = wp.pose
            # STOP_LINE_MARGIN: waypoints behind from stopline so front of cars stops at line
            stop_idx = max(self.stopline_wp_idx - closest_idx - STOP_LINE_MARGIN, 0)
            dist = self.distance(waypoints, i, stop_idx)
            # add linear term for smoother braking (vel -target_vel) = sqrt(2ad)
    ...
    def waypoints_cb(self, waypoints):
        ...
        # Unsubscribe base waypoints as we just need this once time
